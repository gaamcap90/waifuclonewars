...
// normalize possibly-mixed queue entries (ids or objects)
...
function movementCostForTile(tile: HexTile): number {
  ...
  if (tile.terrain.type === "forest") return 2; // forests always cost 2
 ...
 * Returns Map: tileKey -> totalCost from start (including entry cost of destination tile).
 */
...
const useGameState = (gameMode: "singleplayer" | "multiplayer" = "singleplayer") => {
    ...
    const timer = setTimeout(() => {
      ...
      if (gameState.targetingMode && gameState.targetingMode.iconId === activeIcon.id) {
        ...
        for (const enemy of enemies) {
          if (
            hexDistance(activeIcon.position, enemy.position) <=
        ...
        if (!target) {
          ...
          if (
            enemyBase &&
            hexDistance(activeIcon.position, enemyBase.coordinates) <=
              gameState.targetingMode.range
          ) {
          ...
          setGameState((prev) => {
            const targetIcon = prev.players
              ...
              .find(
                (ic) =>
                  ic.isAlive &&
                  ic.position.q === target!.q &&
                  ic.position.r === target!.r
              );
            ...
            if (prev.targetingMode!.abilityId === "basic_attack") {
              if (targetIcon) {
                ...
                const envDamage = Math.max(
                  0.1,
                  calcEffectiveStats(prev, activeIcon).might
                );
                if (target.q === -6 && target.r === 5) {
                  ...
                  if (
                    campIndex !== -1 &&
                    !prev.objectives.beastCamps.defeated[campIndex]
                  ) {
                    const newHp = Math.max(
                      0,
                      prev.objectives.beastCamps.hp[campIndex] - envDamage
                    );
              ...
              // Ability
              const ability = activeIcon.abilities.find(
                (a) => a.id === prev.targetingMode!.abilityId
              );
              if (ability) {
                if (targetIcon) {
                  const dmg =
                  ...
                  const envDamage = Math.max(
                    0.1,
                    calcEffectiveStats(prev, activeIcon).power
                  );
                  if (target.q === -6 && target.r === 5) {
                    updatedBaseHealth[0] = Math.max(0, prev.baseHealth[0] - envDamage);
                  } else if (target.q === 6 && target.r === -5) {
                    ...
                    if (
                      ...
                      if (newHp <= 0 && !defArr[campIndex]) {
                        ...
                        toast.success(
                          "Beast Camp defeated! Team gains +15% Might and Power!"
                        );
      ...
      const enemies = gameState.players[0].icons.filter((i) => i.isAlive);
      const attackRange =
        activeIcon.name === "Napoleon-chan" || activeIcon.name === "Da Vinci-chan"
          ? 2
          : 1;
      ...
      for (const enemy of enemies) {
        if (
          hexDistance(activeIcon.position, enemy.position) <= attackRange &&
      ...
      if (!activeIcon.movedThisTurn && activeIcon.stats.movement > 0) {
        setGameState((prev) => {
          const me = prev.players
            .flatMap((p) => p.icons)
            .find((i) => i.id === activeIcon.id);
          if (!me) return prev;
          ...
          let best:
            | { coord: Coordinates; score: number; cost: number }
            | null = null;

          for (const [key, cost] of costMap.entries()) {
            const [qStr, rStr] = key.split(",");
            const cand: Coordinates = {
              q: parseInt(qStr, 10),
              r: parseInt(rStr, 10),
            };
        ...
        if (prev.targetingMode.abilityId === "basic_attack") {
          if (targetIcon) {
            ...
            if (baseTile) {
              ...
              updatedBaseHealth[enemyId] = Math.max(
                0,
                prev.baseHealth[enemyId] - envDamage
              );
            } else {
              // Beast camp
          ...
          if (typeof ability.damage === "number" && ability.damage > 0) {
            ...
            // non-damaging, non-heal ability: just consume mana + mark used (handled below)
          }
        }
  ...
  const endTurn = useCallback(() => {
    ...
    setGameState((prev) => {
      ...
      if (roundBoundary) {
        // Mana: +1 baseline + adjacency, capped to +4 total gain per round, and overall cap 20
        mana = mana.map((m, pid) => {
          const adj = countAlliesAdjacentToCrystal(
            { ...prev, players: playersAfterRespawn } as GameState,
            pid
          );
